...
- **T1.1 🟡 Week plan CRUD** — สร้าง/แก้ `week_plans` (week_start, price_per_day, deadline=พฤหัส, note), เปลี่ยน status draft→open→closed→billed
  ...
  - Depends on: Phase 0
- **T1.2 🟡 Daily menus editor** — ใส่เมนู จ–ศ (name/description/image), อัปรูปเมนู (storage)
  - AC: ครบ 5 วัน/สัปดาห์, แก้ไขได้, รูปแสดงผล
  - Depends on: T1.1
- **T1.3 🟢 Admin home list** — รายการสัปดาห์ + สถานะ + ปุ่มจัดการ
  - Depends on: T1.1
...
- **T4.1 🟡 สรุปแม่ครัวรายวัน** — เลือกวัน → นับไข่แต่ละชนิด (+สุก/ไม่สุก) + หัว + note พิเศษ + ปุ่มคัดลอกข้อความสรุป
  - AC: ตัวเลขตรงกับ order_items, คัดลอกไปวางในแชตได้
  - Depends on: Phase 2
...
- สมาชิกโหวต 1 เสียง/วัน, สรุปผล, แปลงผู้ชนะเป็น `daily_menus` ของเดือนถัดไป
- (อาจใช้สกิล `claude-api` ช่วยแนะนำ/จัดกลุ่มเมนูจากประวัติ)
